 ...
 ***********************************************/
void Rdf2D::initRDFxy(class CMolecularSystem &molSys, double binwidth,
                      double max_radius) {
 ...
 ***********************************************/
void Rdf2D::getBins() { this->nbin = int(this->max_radius / this->binwidth); }
...
int Rdf2D::getNatoms(class CMolecularSystem &molSys, int typeI, int typeJ) {
  ...
  for (int iatom = 0; iatom < molSys.parameter->nop; iatom++) {
    if (molSys.molecules[iatom].type == typeI ||
        molSys.molecules[iatom].type == typeJ) {
      nop += 1;
    }
  }
 ...
 ***********************************************/
void Rdf2D::getNatomsXY(class CMolecularSystem &molSys, double z_layer,
                        double dz, int typeI, int typeJ) {
  int n_iatoms = 0;
  int n_jatoms = 0;
  ...
  if (typeI == typeJ) {
    ...
    for (int iatom = 0; iatom < molSys.parameter->nop; iatom++) {
      z_atom = molSys.molecules[iatom].get_posz(); // z coord of iatom
      if (this->atomInsideLayer(z_atom, z_layer, dz) == false) {
        continue;
      } // Skip if the atom isn't inside the layer
 ...
 ***********************************************/
void Rdf2D::accumulateRDFxy(class CMolecularSystem &molSys, double z_layer,
                            double dz, int typeI, int typeJ) {
  // Check to make sure that the user has entered the correct type ID
  if (this->typeI != -1 && this->typeI != typeI && nframes > 0) {
    std::cerr << "Type A cannot be changed after init\n";
  }
  if (this->typeJ != -1 && this->typeJ != typeJ && nframes > 0) {
    std::cerr << "Type B cannot be changed after init\n";
  }
  ...
  if (typeI == typeJ) {
    this->histogramRDFxyII(molSys);
  } // for I-I similar type RDF calculations
  else {
    this->histogramRDFxyIJ(molSys);
  } // for I-J similar type RDF calculations
 ...
 *  Calculates the 2D radial distribution function for a single snapshot for a
 ...
 of the same type.
 ***********************************************/
void Rdf2D::singleRDFxy(class CMolecularSystem &molSys, double z_layer,
                        double dz, int typeI, int typeJ) {
  ...
  for (int i = 0; i < n_iatoms - 1; i++) {
    ...
    for (int j = i + 1; j < n_iatoms; j++) {
      ...
      if (dr < this->max_radius) {
        ...
        this->rdf2D[ibin] +=
            2.0 / norm_factor; // Add to histogram for both iatom and jatom
      }
    }
  }
}
...
void Rdf2D::histogramRDFxyIJ(class CMolecularSystem &molSys) {
  int n_iatoms =
      this->n_iatoms; // Total number of atoms of type I (Central atom type)
  int n_jatoms =
      this->n_jatoms; // Total number of atoms of type J (Distribution atom type)
  ...
  for (int i = 0; i < n_iatoms; i++) {
    ...
    for (int j = 0; j < n_jatoms; j++) {
      ...
      if (molSys.molecules[iatom].molID == molSys.molecules[jatom].molID) {
        continue;
      }
      ...
      if (dr < this->max_radius) {
        ibin = int(dr / this->binwidth); // Find which bin the particle falls in
        this->rdf2D[ibin] +=
            1 / norm_factor; // Add to histogram for both iatom and jatom
      }
    }
  }
}
...
void Rdf2D::normalizeRDF2D(double dr) {
  ...
  for (int ibin = 0; ibin < this->nbin; ibin++) {
    ...
    bin_volume =
        dr * bin_area * PI; // Number density rho has already been multiplied
    // Normalization
 ...
 ***********************************************/
double Rdf2D::absDistanceXY(int iatom, int jatom,
...
void Rdf2D::printRDF2D() {
  ...
  this->printToFile(this->nbin, this->rVal, this->rdf2D, "rdf2D",
                    "Radial Distance", "RDF");
}
...
void Rdf2D::checkParameterXY(class CMolecularSystem &molSys) {
  ...
  if (radius > half_box || radius <= 0.0) {
    std::cerr << "I will now set the maximum radius to half the simulation box "
                 "length "
              << "\n";
...
void Rdf2D::checkVolume(class CMolecularSystem &molSys) {
  ...
  if (volume == -1.0) {
    std::cout << "Since the volume has not been set by the user, the "
                 "simulation box volume will be used.\n";
    this->volume = boxx * boxy * boxz;
  } else if (volume <= 0.0 || volume > boxx * boxy * boxz) {
    std::cerr << "The volume entered cannot be used. I will use the simulation "
                 "box volume instead. \n";
    this->volume = boxx * boxy * boxz;
  }
}
...
double Rdf2D::smallest(double x, double y) { return std::min(x, y); }
 ...
 *  Initializes the histogram array for 2-D RDF in the YZ plane
 ...
 values are half the simulation box and the volume of the simulation box respectively
 ***********************************************/
void Rdf2D::initRDFyz(class CMolecularSystem &molSys, double binwidth,
                      double volume, double max_radius) {
  // Get the binwidth, max_radius and volume
  this->binwidth = binwidth;
  this->max_radius = max_radius;
  this->volume = volume;
 ...
 *  Calculates the 2D radial distribution function for a single snapshot for a
 ...
 of the same type.
 ***********************************************/
void Rdf2D::singleRDFyz(class CMolecularSystem &molSys, double x_layer,
                        double dx, int typeI, int typeJ) {
  // There is only one snapshot
  this->nframes = 1;
 ...
 *  Calculates the 2D radial distribution function for a number of snapshots
 ...
 after accumulating to get the RDF 
 ***********************************************/
void Rdf2D::accumulateRDFyz(class CMolecularSystem &molSys, double x_layer,
                            double dx, int typeI, int typeJ) {
  // Check to make sure that the user has entered the correct type ID
  if (this->typeI != -1 && this->typeI != typeI && nframes > 0) {
    std::cerr << "Type A cannot be changed after init\n";
 ...
 ***********************************************/
void Rdf2D::histogramRDFyz(class CMolecularSystem &molSys, double x_layer,
                           double dx) {
  ...
  for (int iatom = 0; iatom < natoms - 1; iatom++) {
    ...
    if (this->atomInsideLayer(x_atom, x_layer, dx) == false) {
      continue;
    }

    // Loop through the j^th atom
    for (int jatom = iatom + 1; jatom < natoms; jatom++) {
      if (molSys.molecules[jatom].type != typeJ && typeJ != -1) {
        continue;
      }
      x_atom = molSys.molecules[iatom].get_posx();
      if (this->atomInsideLayer(x_atom, x_layer, dx) == false) {
      ...
      if (dr < this->max_radius) {
        ...
        // my intuition/Prerna
        this->rdf2D[ibin] +=
            (2.0 / nop_layer); // Add to histogram for both iatom and jatom
      }
    }
  }
}
 ...
 ***********************************************/
int Rdf2D::getNatomsYZ(class CMolecularSystem &molSys, double x_min,
                       double x_max) {
  int nop = 0; // No. of atoms
  ...
  if (nop == 0) {
    std::cerr << "There are no atoms of type" << typeI << "and" << typeJ
              << "inside the x range given\n";
    return molSys.parameter->nop;
  }
 ...
 ***********************************************/
double Rdf2D::absDistanceYZ(int iatom, int jatom,
                            class CMolecularSystem &molSys) {
  double dr[2]; // Relative distance between wrapped coordinates
 ...
 ***********************************************/
int Rdf2D::binsInRDF() { return this->nbin; }
