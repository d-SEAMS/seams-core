 ...
 ***********************************************/
void Rdf3D::initRDF3D(class CMolecularSystem &molSys, double binwidth,
 ...
 ***********************************************/
void Rdf3D::getBins() { this->nbin = int(this->max_radius / this->binwidth); }
 ...
 ***********************************************/
void Rdf3D::accumulateRDF3D(class CMolecularSystem &molSys, int typeI,
                            int typeJ) {
  // Check to make sure that the user has entered the correct type ID
  if (this->typeI != -1 && this->typeI != typeI && nframes > 0) {
    std::cerr << "Type I cannot be changed after init\n";
  }
  if (this->typeJ != -1 && this->typeJ != typeJ && nframes > 0) {
    std::cerr << "Type J cannot be changed after init\n";
  }
  ...
  if (typeI == typeJ) {
    this->histogramRDF3Dii(molSys);
  } // for I-I similar type RDF calculations
  else {
    this->histogramRDF3Dij(molSys);
...
void Rdf3D::normalizeRDF3D() {
  ...
  double rho =
      this->n_jatoms / this->volume; // Number density of distribution atoms J
  // Loop over all bins
 ...
 ***********************************************/
double Rdf3D::getAbsDistance(int iatom, int jatom,
...
void Rdf3D::printRDF3D() {
  ...
  this->printToFile(this->nbin, this->rVal, this->rdf3D, "rdf3D",
                    "Radial Distance", "RDF");
}
...
double Rdf3D::smallest(double x, double y) { return std::min(x, y); }

/********************************************/ /**
 *  Returns the number of bins in the RDF array
 ***********************************************/
int Rdf3D::binsInRDF() { return this->nbin; }
