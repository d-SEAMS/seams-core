...
class GenEigsBase {
 ...
 private:
  ...
  Index nev_adjusted(Index nconv) {
    ...
    if (nev_new > m_ncv - 2) nev_new = m_ncv - 2;
 ...
 protected:
  ...
  virtual void sort_ritzpair(int sort_rule) {
    ...
    switch (sort_rule) {
      ...
      case SMALLEST_MAGN: {
        SortEigenvalue<Complex, SMALLEST_MAGN> sorting(m_ritz_val.data(),
                                                       m_nev);
        ind = sorting.index();
      } break;
      case SMALLEST_REAL: {
        SortEigenvalue<Complex, SMALLEST_REAL> sorting(m_ritz_val.data(),
                                                       m_nev);
        ind = sorting.index();
      } break;
      case SMALLEST_IMAG: {
        SortEigenvalue<Complex, SMALLEST_IMAG> sorting(m_ritz_val.data(),
                                                       m_nev);
        ind = sorting.index();
      } break;
      default:
