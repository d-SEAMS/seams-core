...
seams-core/
  ...
  tests/                 Catch2 binaries and walk_compare
  lean/                  Lean 4 model of ringAdjacentCompletion
...
- ~cageAffiliation~ / ~seededCageAffiliation~ :: Ice score on
  ...
  The fifth argument of ~seededCageAffiliation~ is
  ~ringAdjacentCompletion~ (default ~false~; ~seams cages
  --complete~ sets it). See
...
~tests/walk_compare~ is a Meson executable. Inputs and columns live
on [[file:../reference/walk-compare.org][walk_compare]].

Lean 4 statements about ~ring::ringAdjacentCompletion~ live in
=lean/=. See [[file:lean.org][Lean proofs]].
