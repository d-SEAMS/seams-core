...
The pixi environment provides all build dependencies: a C++ compiler, Eigen,
BLAS/LAPACK, Catch2, Meson, Ninja, nanobind, NumPy, and pytest.
...
~pixi run setup~ configures a Meson build directory at ~bbdir/~ with tests and
Python bindings enabled. ~pixi run build~ compiles the C++ library and the
...
~pixi run test~ runs the Catch2 test suite through Meson. ~pixi run pytest~ first
installs the Python package, then runs pytest on ~tests/python/~.
...
seams-core/
  src/                   C++ source files
    include/
      ...
      external/          Vendored headers (backward, cxxopts, rang, ...)
    ...
    ...
  python/
    bindings.cpp         nanobind bindings (_core module)
    pydseamslib/
      __init__.py        Python package entry point
      adapters/
        _ase.py          ASE Atoms to PointCloud conversion
  tests/
  ...
  pyproject.toml         Python package metadata (meson-python backend)
#+end_src
