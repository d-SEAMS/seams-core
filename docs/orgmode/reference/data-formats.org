...
~seams read~ prints the three bound spans (~box Lx Ly Lz~). It does
not print the tilt. The cloud still keeps ~box[3..5]~ when the dump
had them.

Mixed water dumps in this tree use type 2 for oxygen and type 1 for
hydrogen. Single-site mW dumps in this tree use type 1. A mixed-type
...
[[file:../howto/ionic-liquid.org][Pick one type]].
