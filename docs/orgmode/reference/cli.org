...
cages | rdf | cn | hbonds | pairs | density-z | domains~. ~chill_plus~
is an accepted alias of ~chill-plus~ (not listed in that help string).
...
only by ~cages~. ~--complete~ is used by ~cages~ and ~ions~: it
