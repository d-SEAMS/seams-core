...
is tried first (~-mp=gpu~, then ~-mp=gpu -gpu=cc80~). clang with
libomptarget is the fallback (~-fopenmp
...
GPU batch shells do not have Lmod on ~PATH~. Use the OpenHPC
nvc++ prefix and the GCCcore-13.3.0 EasyBuild prefixes for Meson,
Ninja, Eigen, FlexiBLAS, and Python 3.12. Cluster Catch2 is 2.x;
the tree carries a Catch2 3 wrap. CUDA 12.4 supplies ~nsys~ 2023.4.
...
export LD_LIBRARY_PATH=$EB/Python/3.12.3-GCCcore-13.3.0/lib:$NVHPC_ROOT/compilers/lib:$NVHPC_ROOT/cuda/lib64:$NVHPC_ROOT/math_libs/lib64:$EB/GCCcore/13.3.0/lib64:$EB/FlexiBLAS/3.4.4-GCC-13.3.0/lib:$EB/Z3/4.13.0-GCCcore-13.2.0/lib
...
export LDFLAGS="-B$PWD/elja-crt -L$PWD/elja-crt -L/lib64"

# nvc++ 22.3 reports no C++20. Clang 17 with libomptarget is the
# compiler that accepts cpp_std=c++20 and the offload probe.
export CC=$EB/Clang/17.0.6-GCCcore-13.2.0/bin/clang
export CXX=$EB/Clang/17.0.6-GCCcore-13.2.0/bin/clang++
export CPATH=$PWD/elja-sysroot/usr/include
