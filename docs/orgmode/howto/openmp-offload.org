...
~GCC/13.3.0~ and ~nvidia/nvhpc/22.3~ both claim Lmod's compiler
family, so they are not loaded together. Load nvhpc, then put the
GCCcore-13.3.0 Meson, Ninja, Eigen, and FlexiBLAS prefixes on
~PATH~ and ~PKG_CONFIG_PATH~. Cluster Catch2 is 2.x; the tree
carries a Catch2 3 wrap. CUDA 12.4 supplies ~nsys~ 2023.4.
...
export PATH=$EB/Meson/1.4.0-GCCcore-13.3.0/bin:$EB/Ninja/1.12.1-GCCcore-13.3.0/bin:$EB/CUDA/12.4.0/bin:$PATH
export PKG_CONFIG_PATH=$EB/Eigen/3.4.0-GCCcore-13.3.0/share/pkgconfig:$EB/FlexiBLAS/3.4.4-GCC-13.3.0/lib/pkgconfig
export LD_LIBRARY_PATH=$EB/FlexiBLAS/3.4.4-GCC-13.3.0/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}
...
#+begin_src bash
srun --partition=gpu-1xA100 --gres=gpu:1 --account=chem-ui \
