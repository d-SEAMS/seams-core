...
Pass an ASE ~Atoms~ to ~pydseamslib.from_ase~. Oxygen is selected by
...
- ~from_ase~ does not read a LAMMPS dump header. The cell comes from
  ~atoms.cell~; the origin from ~atoms.get_celldisp()~.
- ~to_ase~ writes ~arrays['ice_type']~ after ~chill~ / ~chill_plus~.
- The old ~pydseamslib.adapters._ase.to_pointcloud~ helper is gone.
  Use ~from_ase~.
